...
-- ============================================
SELECT 
    '🔍 VERIFICACIÓN DE DUPLICADOS' as titulo;

-- Buscar servicios con nombres duplicados
SELECT 
    nombreServicio,
    COUNT(*) as cantidad,
    GROUP_CONCAT(idServicio ORDER BY idServicio) as ids_duplicados,
    CASE 
...
SELECT 
    CASE 
        ...
        THEN '✅ No se encontraron servicios duplicados'
...
-- ============================================
SELECT 
    '📋 SERVICIOS DISPONIBLES' as titulo;
...
SELECT 
    ...
    CASE 
        WHEN s.nombreServicio IS NULL OR TRIM(s.nombreServicio) = '' THEN '❌ SIN NOMBRE'
        ELSE '✅ OK'
    END as validacion
FROM servicio s
LEFT JOIN familiaservicio fs ON s.idFamilia = fs.idFamilia
ORDER BY fs.nombreFamilia, s.nombreServicio;

-- ============================================
-- 3. CONTAR SERVICIOS POR FAMILIA
-- ============================================
SELECT 
    '📊 ESTADÍSTICAS POR FAMILIA' as titulo;
...
GROUP BY fs.idFamilia, fs.nombreFamilia
...
-- ============================================
SELECT 
    '⚠️ SERVICIOS HUÉRFANOS (sin familia asignada)' as titulo;
...
SELECT 
    CASE 
        WHEN (SELECT COUNT(*) FROM servicio WHERE idFamilia IS NULL) = 0 
        THEN '✅ Todos los servicios tienen familia asignada'
        ELSE CONCAT('⚠️ Hay ', (SELECT COUNT(*) FROM servicio WHERE idFamilia IS NULL), ' servicios sin familia')
...
-- ============================================
SELECT 
    '📁 FAMILIAS DE SERVICIOS DISPONIBLES' as titulo;
...
SELECT 
    'Total de Servicios' as metrica,
    COUNT(*) as valor
FROM servicio
UNION ALL
SELECT 
    'Servicios con Familia' as metrica,
    COUNT(*) as valor
FROM servicio
WHERE idFamilia IS NOT NULL
UNION ALL
SELECT 
    'Servicios sin Familia' as metrica,
    COUNT(*) as valor
FROM servicio
WHERE idFamilia IS NULL
UNION ALL
SELECT 
    'Total de Familias' as metrica,
    COUNT(*) as valor
FROM familiaservicio
UNION ALL
SELECT 
    'Familias con Servicios' as metrica,
